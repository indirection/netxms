         ...
         {            
            ...
            if (it.hasNext())
            {
               while(it.hasNext())
               {
                  ...
                  final SshKeyData key = (SshKeyData) it.next();
                  try
                  {
                  ...
                  {
                     if (e.getErrorCode() != RCC.RCC_SSH_KEY_IN_USE)
                        throw e;
                     ...
                     getDisplay().syncExec(new Runnable() {
                        ...
                        public void run()
                        {
                           //TODO: show list of nodes that use this certificate (already added to reply message
                           retry[0] = MessageDialogHelper.openQuestion(getSite().getShell(), "Confirm Delete",
                                 String.format("SSH key \"%s\" is in use. Are you sure you want to delete it?", key.getName()));
                        }
                     });
                     if (retry[0])
                     {
